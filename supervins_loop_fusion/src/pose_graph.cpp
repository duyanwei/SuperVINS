...
Database PoseGraph::initDb(Vocabulary voc)
{
...
int PoseGraph::detectLoop(KeyFrame *keyframe, int frame_index)
{
    ...
    if (DEBUG_IMAGE)
    {
        int feature_num = keyframe->SuperPointDescriptors.rows;
    ...
    dpl_db.query(keyframe->SuperPointDescriptors, ret, 4, frame_index - 50);

    cout << "Searching for Image " << frame_index << ". " << ret << endl;

    TicToc t_add;
    dpl_db.add(keyframe->SuperPointDescriptors);
